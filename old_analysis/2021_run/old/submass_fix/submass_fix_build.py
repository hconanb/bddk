...
def submass_harris(infilename, intreename, flag):
    ...
    tl_sb = ["0","1","2","3"]
    columns_to_read_in = ["D{1,2}_P{X,Y,Z,E}", "KSTH*{1,2}_P{X,Y,Z,E}", "D1_M","D2_M", "B_*" ]
    nlist = [2]
    nsblist = [2,3]
    ...
    df = df.apply(get_fix_id,  args=("D1KSTH1","B_M02","B_M03","B_M12","B_M13"), axis=1, result_type='expand')
    print(f"Saving {flag}_allyear_fixed.root")
    df[Final_n_List].to_root(f"rootfiles/{flag}_allyear_fixed.root", key=f"{intreename}", store_index=False)
    print(f"Saving {flag}_allyear_fixed.root")
