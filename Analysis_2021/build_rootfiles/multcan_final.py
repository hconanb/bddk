...
def getBestCand(type, year, spec):
    ...
    with uproot.open(f"{dsig_file}:DecayTreeTuple") as sigin, uproot.open(f"{dsb_file}:DecayTreeTuple") as sbin:
        ...
        if type == "mc" or "norm" in spec:
            ...
            print(len(df_sig_output.index), len(df_T.index), len(df_nTaT.index),)
        ...
        text_df.to_csv(f"/mnt/c/Users/Harris/Google Drive/LHCb/bddk/Analysis_2021/mc_efficiencies/multi_can_effs/{spec}_{year}_{type}_cc.txt")


#dataz_c, zz_c, p_c, m_c, st_c,
#, 2017, 2018
for year in [2016]:
    for cspec in [z_c]:
...
# for cspec in mc_spec_list:
#     for year in [2016, 2017, 2018]:
#         getBestCand("mc", year, cspec)
