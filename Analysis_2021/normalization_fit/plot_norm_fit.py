...
def plot_fit(var, frame, folder, name, fancy_string):
    ...
    save_png(p, folder, name, rpflag = 1)

for spec in ["norm7","norm8"]:
    for year in ["2016","2017","2018"]:
        for trigger in ["T","nTaT"]:
            base_file = ROOT.TFile(f"base_norm_files/{spec}_{year}_{trigger}.root")
            ...
            plot_fit(b_dtf_m, frame, "norm_fit", f"{spec}_{year}_{trigger}_fit", fancy_string)
